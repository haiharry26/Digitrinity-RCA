...
function loadMultiSelectClass(contextPath,academicYearId,schoolBoardId) {
...
$.ajax({  
	...
	data : JSON.stringify(JSONObject),
	success: function(response){
		successLoadMultiSelectClass(response);
	},  
	error: function(e){  
		fail(e)
	}  
...
function successLoadMultiSelectClass(response) {
...
function  checkedClass() { 
	var classIdArr = [];
	var selectedClassId = [];
	classIdArr =document.getElementById("classIdArrfield").value;
	
	 var selectedClassArr = [];  
		
	  selectedClassArr =classIdArr.split(',');
	 selectedClassId.push(selectedClassArr);
	...
	if( selectedClassArr.length > 0){
		...
		for (index = 0; index < selectedClassArr.length; index++) { 
